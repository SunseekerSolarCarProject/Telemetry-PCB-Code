...
#define TELEM_STATUS_COUNT	8			// Number of ticks per event: 10 sec
#define HS_COMMS_SPEED      8            // Number of ticks per event:  0.5 sec
...
#define MODEM_BR1 38400
#define MODEM_UCBRS1 0x06 // 2*ROUND([SMCLK_RATE/MODEM_BR-INT(SMCLK_RATE/MODEM_BR)]*8)
#endif
...
#define HF_MSG_PACKET  5    //number of messages per packet in high frequency
...
#define No_MSG_PACKET  29    //number of messages that we receive and don't send out
...
static int addr_lookup[LOOKUP_ROWS][5] = {
  ...
  {MC_CAN_BASE1 + MC_LIMITS,	      8,                0x0100,                   3,							15},                    //1-0x01        High = Active Motor              Low = Error & Limit flags
  ...
  {DC_CAN_BASE + DC_DRIVE,            2,                0x0800,                   3,							23},                    //29-0x01	    High = Motor Current Setpoint    Low = Motor Velocity Setpoint
  ...
  {DC_CAN_BASE + DC_SWITCH,           3,                0x0080,                   3,							25},                    //32-0x04	    High = Switch position           Low = Switch state change
  {BP_CAN_BASE,                      14,                0x2000,                   3,							43},                    //33-0x580    High = BPV2" string or nulls     Low = CAN1_SERIAL Number
  {BP_CAN_BASE + BP_VMAX,             3,                0x0008,                   0,							11},                    //34-0x01       High = Max Voltage               Low = Cell Number
  {BP_CAN_BASE + BP_VMIN,             4,                0x0010,                   0,							12},                    //35-0x02	    High = Min Voltage               Low = Cell Number
  ...
  {BP_CAN_BASE + BP_ISH,              2,                0x0004,                   0,						     5},                    //38-0x05	    High = Shunt Current  			 Low = Battery Voltage
