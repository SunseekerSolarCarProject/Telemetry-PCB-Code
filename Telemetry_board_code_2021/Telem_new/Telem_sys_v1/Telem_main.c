...
int main(void) {
	...
	// Code to be executed once to install initial time to device.
	// Battery must be installed
	//init_MCP7940M();
	//delay();

	// Retrieve RTC IC time and store in MSP RTC
    ...
    while(1)
    {
    	...
    	if(status_flag){
    		...
    		//get_MCP7940M(&bhrs,&bmin,&bsec); 	//polling
    		//get_MCP7940M_int();				//interrupt based
    		//insert_time_2(&pckHF.timexmit.time_msg[0]);

        	//MSP RTC time stamp and packet trasnmission by interrupt
    		getRTCTime(&thrs,&tmin,&tsec);
    	...
    	// Periodic interrogation of alternating MPPTs
    	...
    	if(AC_comm_flag == TRUE){
			...
			can_fifo_PUT(&can0_queue, TX_can0_message);
