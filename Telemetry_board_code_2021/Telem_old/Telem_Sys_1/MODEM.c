...
void MODEM_init()
{
	UART1CTL1 |= UCSWRST | UCSSEL_2;	//put state machine in reset & SMCLK
	UART1CTL0 |= UCMODE_0;

	UART1BRW = (int)(SMCLK_RATE/MODEM_BR1);	//16MHZ/115200
	UART1MCTL |= UCBRF_0 + (char)MODEM_UCBRS1; //modulation UCBRSx=computed from baud rate UCBRFx=0

	UART1IFG &= ~UCTXIFG;			//Clear Xmit and Rec interrupt flags
	UART1IFG &= ~UCRXIFG;
	UART1CTL1 &= ~UCSWRST;			//initalize state machine
	UART1ABCTL |= UCABDEN;
