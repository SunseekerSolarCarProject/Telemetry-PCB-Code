...
void Port_Init(void)
{
    ...
    P2SEL0 |= XT1IN | XT1OUT;                               // Set P2.0 & P2.1 as XT1IN/XT1OUT peripheral
    P2DIR |= XT1OUT;
    P2DIR &= ~XT1IN;
...
void Clock_XT1_Init(void)
{
    ...
    CSCTL6 &= ~(XT1DRIVE1 | XT1DRIVE0);                             // Lowest drive current LF 32KHz oscillator
    ...
    CSCTL5 = DIVS_2;                                                // sets SMCLK source to be 4mhz
/*
    P1DIR |= BIT7 | BIT3;
    P1SEL1 |= BIT7 | BIT3;                                          // set SMCLK P1.7 pin as second function
                                                                    // set MCLK on p1.3 temporary for testing
    P2DIR |= BIT2;
